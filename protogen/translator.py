    ...
    for file in files:
        ...
        with open(file, "r") as f:
            while line := f.readline():
                ...
                line = re.sub(r"DEFAULT '*([a-z_\(\)]+)'*",r"DEFAULT: '\1',",line,flags=re.IGNORECASE)
                line = re.sub(r"DEFAULT (\d+)",r"DEFAULT: \1,",line,flags=re.IGNORECASE)
        ...
        with open(file,"r") as f:
            while line := f.readline():
                ...
                if table_name := re.match(
                    ...
                    if col := re.match(
                            r"^\s*(?P<column>\w+)\s+(?P<type>[\w\(\)]+)\s+(?P<args>.*)",
                            line, re.IGNORECASE
                        ):
...
if __name__ == "__main__":
    ...
    full_path = os.path.join(os.path.dirname(__file__),f"../{args.dir}")
    ...
    for file in ls:
        if ".sql" not in file: continue
        files.append(f"{full_path+("" if is_dir else "/")}{file}")
